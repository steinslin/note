...
随着React Vue等框架的流行，Virtual DOM也越来越火，snabbdom是其中一种实现，而且Vue 2.x版本的Virtual DOM部分也是基于snabbdom进行修改的。snabbdom这个库核心代码只有200多行，非常适合想要深入了解Virtual DOM实现的读者阅读。如果您没听说过snabbdom，可以先看看[官方文档](https://github.com/snabbdom/snabbdom)。
...
> vue使用了snabbdom，读懂snabbdom对理解vue的实现有帮助

### 什么是Virtual DOM

snabbdom是Virtual DOM的一种实现，所以在此之前，你需要先知道什么是Virtual DOM。通俗的说，Virtual DOM就是一个js对象，它是真实DOM的抽象，只保留一些有用的信息，更轻量地描述DOM树的结构。 比如在`snabbdom`中，是这样来定义一个`VNode`的
...
从上面的定义我们可以看到，我们可以用js对象来描述`dom`结构，那我们是不是可以对两个状态下的js对象进行对比，记录出它们的差异，然后把它应用到真正的dom树上呢？答案是可以的，这便是`diff`算法，算法的基本步骤如下：

- 用js对象来描述dom树结构，然后用这个js对象来创建一棵真正的dom树，插入到文档中
- 当状态更新时，将新的js对象和旧的js对象进行比较，得到两个对象之间的差异
- 将差异应用到真正的dom上
...
通过参数可以知道，这里有接受一个`modules`数组，另外有一个可选的参数`domApi`，如果没传递会使用浏览器中和dom相关的api，具体可以看[这里](https://github.com/snabbdom/snabbdom/blob/master/src/htmldomapi.ts)，这样的设计也很有好处，它可以让用户自定义平台相关的api，比如可以看看[weex的相关实现](https://github.com/vuejs/vue/blob/dev/src/platforms/weex/runtime/node-ops.js)
...
function updateChildren(parentElm: Node, oldCh: Array<VNode>, newCh: Array<VNode>, insertedVnodeQueue: VNodeQueue) {
  ...
  while (oldStartIdx <= oldEndIdx && newStartIdx <= newEndIdx) {
    ...
    if (oldStartVnode == null) {
      ...
      // 2. 循环已经执行过一部分了，因为每次比较结束后，下标都会向中间靠拢，而且每次都会处理一个节点，这时下标左右两边已经处理完成，可以把下标开始到结束区域当成是并未开始循环的一个整体，所以插入到oldEndVnode后面是合理的（在当前循环来说，也相当于是最后面，同1）
      ...
      if (isUndef(idxInOld)) { // New element
        ...
        if (elmToMove.sel !== newStartVnode.sel) {
          ...
          // 在oldCh中将当前已经处理处理的的vnode置空，等循环到这个下标的时候直接跳过
...
2. 第一轮比较：开始结束节点两两并不相等，于是看newStartVnode在旧节点中是否存在，最后找到了在第二个位置，调用patchVnode进行更新，将oldCh[1] 至空，将dom插入到oldStartVnode前面，newStartIdx向中间移动，状态更新如下

![snabbdom-2](./imgs/snabbdom-2.jpeg)

3. 第二轮比较：oldStartVnode 和 newStartVnode 相等，直接patchVnode，newStartIdx和oldStartIdx向中间移动，状态更新如下

![snabbdom-3](./imgs/snabbdom-3.jpeg)

4. 第三轮比较：oldStartVnode为空，oldStartIdx向中间移动，进入下轮比较，状态更新如下

![snabbdom-4](./imgs/snabbdom-4.jpeg)

5. 第四轮比较：oldStartVnode 和 newStartVnode 相等，直接patchVnode，newStartIdx和oldStartIdx向中间移动，状态更新如下

![snabbdom-5](./imgs/snabbdom-5.jpeg)

6. oldStartVnode 和 newStartVnode 相等，直接patchVnode，newStartIdx和oldStartIdx向中间移动，状态更新如下

![snabbdom-6](./imgs/snabbdom-6.jpeg)

7. 循环结束，由于是旧节点先结束循环而且还有没处理的新节点，处理剩下的新节点
...
这两个函数主要用来添加vnode和移除vnode，代码逻辑基本都能看懂。

#### thunk 函数

一般我们的应用是根据js状态来更新的，比如下面这个例子
...
对于这种情况，`snabbdom`提供了一种优化手段，也就是`thunk`，该函数同样返回一个`vnode`节点，但是在`patch`开始之前，会对参数进行一次比较，如果相同，将不会进行`patch`，这个有点类似于`React`的`pureComponent`，`pureComponent`的实现上会做一次浅比较`shadowEqual`，结合`immutable`数据进行使用。上面的例子可以变成这样
...
可以回顾下patchVnode的实现，在prepatch后，会对vnode的数据做比较，比如当`children`相同、`text`相同都会结束`patchVnode`。
