...
const components = utils.readComponents('components');
const utilList = utils.readUtils('components/_utils');
...
function genConfig() {
  const externals = {};
  const external = ['vue', 'moment'];
  components.forEach(component => {
    const isDir = component.indexOf('.') === -1;
    const key = `components/${component}`;

    if (isDir) {
      externals[key] = `gs-ui/lib/${component}.js`;
      external.push(key);
    }
  });
  utilList.forEach(item => {
    const name = item.split('.')[0];
    const key = `components/_utils${name}`;

    externals[key] = `gs-ui/lib/_utils${item}`;
    external.push(key);
  });

  config.output.paths = externals;
  config.external = id => {
    return external.indexOf(id) >= 0 || /^babel/.test(id);
  };
  ...
  components.forEach(component => {
    ...
    let _config = utils.deepClone(config);

    if (isDir && component === 'test') {
      _config.input = `components/${component}/index.js`;
      _config.output.file = `lib/${component}.js`;
  ...
  utilList.forEach(item => {
    ...
    _config.output.file = `lib/_utils${name}.js`;
    configs.push(_config);
  })

  let globalConfig = utils.deepClone(config);

  globalConfig.input = 'components/index.js';
  globalConfig.output.file = 'lib/index.js';
  globalConfig.external = id => {
    return /^vue$/.test(id) || /^moment$/.test(id) || /^babel/.test(id);
  }
