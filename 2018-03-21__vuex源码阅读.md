...
接下去是一些内部变量的初始化
`_committing`提交状态的标志，当使用`mutation`时，会先赋值为`true`，再执行`mutation`，修改`state`后再赋值为`false`，在这个过程中，会用`watch`监听state的变化时是否`_committing`为true，从而保证只能通过`mutation`来修改`state`
